...
const char* MQTT_PASSWORD = "";
  ...
  const size_t RFID_CHIP_OFFSET = 4;
  const size_t RFID_CHIP_LEN = 4;
  ...
  bool validRfidChecksum(const uint8_t* frame, size_t len) {
    ...
    // Protocol checksum: XOR length/type/status/chip bytes, indexes 1..7.
  ...
  bool readRFIDBinaryFrame(uint8_t* out, size_t outLen, size_t* frameLenOut) {
    while (RFID.available()) {
      ...
      // 10-byte frame:
      // 02 0A 02 status chip0 chip1 chip2 chip3 xor 03
  ...
  uint32_t chipIdFromBinaryFrame(const uint8_t* frame) {
    return ((uint32_t)frame[RFID_CHIP_OFFSET] << 24) |
           ((uint32_t)frame[RFID_CHIP_OFFSET + 1] << 16) |
           ((uint32_t)frame[RFID_CHIP_OFFSET + 2] << 8) |
           ((uint32_t)frame[RFID_CHIP_OFFSET + 3]);
  }
  ...
  void chipIdDecCStr(uint32_t chipId, char* out, size_t outLen) {
    snprintf(out, outLen, "%0*lu", RFID_MIN_DEC_DIGITS, (unsigned long)chipId);
  }
  ...
  void rfidTask(void* pv) {
    ...
    for (;;) {
      while (true) {
        ...
        uint32_t chipId = chipIdFromBinaryFrame(frame);
        char chipHex[9];
        char chipDec[24];
