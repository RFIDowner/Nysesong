// PigeonPalHexV31 — V3.1 (frame-stats-runden 2026-07-22). Eneste endring fra
// PigeonPalHexV3: RFID-tellerne (framesOk/tooShort/overflow/dupBlocked/
// bytesRead) sendes med i heartbeatet, slik at lesetap kan diagnostiseres fra
// skyen i ettertid: sveip uten app-respons MED framesOk-økning = tap
// oppstrøms; UTEN økning = modul/geometri. Tellerne er kumulative siden boot
// (uptimeS gir referansen). Reader-modulen er urørt — ESP32 lytter bare.
//
// Historikk (fra V3):
// Base: NyTilSesong.ino (som ligger urørt). Hex-parseren er beholdt 1:1:
// STX 0x02 | hex-tegn | ETX 0x03, chip-id = desimal av hex-verdien med minst
// 10 sifre — enheten sender altså IDENTISKE chip-id-er som den alltid har
// gjort; ingenting i duedatabasen trenger re-registrering.
//
// V3-tilleggene er portet 1:1 fra PigeonPalBinaerV3 (2026-07):
...
//     portalen igjen. Rask LED-blink kvitterer mens knappen holdes.
//
// I tillegg to generiske robusthetsgrep fra V2-runden (ikke antennespesifikke):
//  - UART RX-buffer 1024 for burst-toleranse.
//  - Head-of-line-rotasjon ved MQTT ack-timeout (én problem-melding
//    blokkerer aldri nye lesinger).
// Og: serial-logging er koblet inn — hver OK-lesing skriver chipId til
// USB-serial, statslinje hvert 5. sekund (NyTilSesong printet ingenting
// per lesing, som gjorde feltdiagnose i seriemonitor umulig).
...
// ================== KONFIG ==================
const char* FW_VERSION = "3.1.0-hex";
...
const unsigned long HEARTBEAT_INTERVAL_MS = 60000UL;
  ...
  // ================== RFID PARSING (HEX/ASCII — uendret fra NyTilSesong) ==================
  // Rammen: STX 0x02 | hex-tegn (0-9A-Fa-f) | ETX 0x03. CR/LF ignoreres.
  // Chip-id = desimal av hex-verdien, minst 10 sifre — samme som alltid.
  ...
  // V3: skriv faktisk chip-id til seriemonitor per aksepterte lesing —
  // NyTilSesong printet ingenting, som gjorde feltdiagnose umulig.
    ...
    for (;;) {
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        if (mqttConnected &&
          ...
          // V3.1: RFID-tellerne med i heartbeatet (kumulative siden boot).
          // uint32_t-lesing er atomisk på ESP32, så rfidTask kan trygt
          // oppdatere dem samtidig.
          char hb[352];
          snprintf(hb, sizeof(hb),
            "{\"deviceId\":\"%s\",\"fw\":\"%s\",\"uptimeS\":%lu,\"rssi\":%d,\"queued\":%d,\"heapFree\":%u,"
            "\"rfFramesOk\":%lu,\"rfTooShort\":%lu,\"rfOverflow\":%lu,\"rfDupBlocked\":%lu,\"rfBytes\":%lu}",
            deviceId.c_str(), FW_VERSION,
            (unsigned long)(millis() / 1000UL),
            (int)WiFi.RSSI(),
            bufferCountApprox(),
            (unsigned)ESP.getFreeHeap(),
            (unsigned long)rfidFramesOk,
            (unsigned long)rfidFramesTooShort,
            (unsigned long)rfidFramesOverflow,
            (unsigned long)rfidDuplicatesBlocked,
            (unsigned long)rfidBytesRead);
  ...
  void setup() {
    ...
    // Burst tolerance: default RX buffer (256 B) → 1024 B.
