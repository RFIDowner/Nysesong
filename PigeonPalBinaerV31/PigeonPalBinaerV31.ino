// PigeonPalBinaerV31 — V3.1 (frame-stats-runden 2026-07-22). Eneste endring
// fra PigeonPalBinaerV3: RFID-tellerne (framesOk/badLen/badEnd/checksumFail/
// overflow/dupBlocked/bytesRead) sendes med i heartbeatet — lesetap kan
// diagnostiseres fra skyen. checksumFail er særlig interessant for denne
// antennetypen: to chipper i feltet samtidig gir ødelagte XOR-sjekksummer.
// Tellerne er kumulative siden boot (uptimeS gir referansen). Reader-modulen
// er urørt — ESP32 lytter bare.
//
// Historikk (fra V2/V3):
      ...
      if (WiFi.status() == WL_CONNECTED) {
        ...
        if (mqttConnected &&
          ...
          snprintf(hb, sizeof(hb),
            ...
            "\"rfFramesOk\":%lu,\"rfBadLen\":%lu,\"rfBadEnd\":%lu,\"rfChecksumFail\":%lu,"
            "\"rfOverflow\":%lu,\"rfDupBlocked\":%lu,\"rfBytes\":%lu}",
            ...
            (unsigned long)rfidFramesBadLen,
            (unsigned long)rfidFramesBadEnd,
            (unsigned long)rfidFramesChecksumFail,
